...
mdl = pulp.LpProblem('continuous_line')
...
u = dict()
for i in C:
    u[i] = pulp.LpVariable(cat=pulp.LpInteger, lowBound=0, upBound=27, name=f'u_{i}')
# add constraints
# exactly one origin
for i in C:
...
mdl.setObjective(pulp.lpSum(x))  # not required for this problem
# endregion
